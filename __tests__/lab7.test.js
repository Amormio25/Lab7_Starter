describe("Basic user flow for Website", () => {
    ...
    it("Make sure <product-item> elements are populated", async () => {
        ...
        * Right now this function is only checking the first <product-item> it found, make it so that
          ...
          */
        prodItemsData.forEach((prodData) => {
            if (prodData.title.length == 0) {
                allArePopulated = false;
            }
            if (prodData.price.length == 0) {
                allArePopulated = false;
            }
    ...
    it("Checking number of items in cart on screen after reload", async () => {
        ...
        const allRemoveFromCart = await page.$$eval("product-item", (items) =>
            items.every(
                (item) =>
        ...
        const num = await innerText.jsonValue();
        expect(num).toBe("20");
    }, 30000);
    ...
    it("Checking number of items in cart on screen after reload", async () => {
        ...
        const allAddToCart = await page.$$eval("product-item", (items) =>
            items.every(
                (item) =>
                    item.shadowRoot.querySelector("button").innerText ===
                    "Add to Cart",
